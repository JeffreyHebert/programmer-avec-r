...
quelque peu surprenante, c'est un objet de mode \mode{logical}. Cependant,
...
étiquettes sont enregistrées dans l'attribut \attribut{names} du
vecteur. Il y a deux grandes façons de procéder: en spécificant les
noms dès la création du vecteur, ou a posteriori en ajoutant
...
nouvelle fonction dans R à l'aide de la fonction \Fonction{function}.
La syntaxe exacte de la définition d'une fonction est la suivante:
\begin{Schunk}
...
retourne déjà une fonction et si aucun nom n'est attribué à cet objet,
on obtient ce qui est appelé une \emph{fonction anonyme}. Les
fonctions anonymes sont particulièrement utiles avec les fonctions
d'application qui seront étudiées au \autoref{chap:application}.

Une fonction peut n'avoir aucun argument formel ou plusieurs. Il n'y a
pas de limite pratique au nombre d'arguments formels que peut avoir
une fonction. Les arguments formels peuvent être des symboles, des
constructions de la forme \code{symbole = défaut}, ou encore
l'argument formel spécial `\code{...}'.
...
de l'argument \code{symbole}. En d'autres termes, c'est la valeur de
l'argument si aucune valeur n'est spécifiée dans l'appel de la
fonction.
<<echo=TRUE>>=
...
exemple à la \autoref{bases:exemples} que ce que nous pourrions faire
...
Nous avons déjà évoqué certaines des règles qui régissent les appels
de fonctions dans R. Regardons maintenant cela de plus près. Aux fins
d'illustrations, nous définissons une petite fonction comptant trois
arguments, dont seul le premier n'a pas de valeur par défaut.
<<echo=TRUE>>=
f <- function(x, y = 0, p = 2) sum(abs(x - y)^p)
@

L'ordre des arguments dans la définition de la fonction est important.
Nous avons deux choix au moment d'appeler une fonction: spécifier les
valeurs des arguments dans le bon ordre, ou nommer explicitement les
arguments avec une construction de la forme \code{symbole =
  expression}, auquel cas l'ordre n'importe plus. Il est beaucoup plus
prudent et \emph{fortement recommandé} de spécifier les arguments par
leur nom, surtout après les deux ou trois premiers arguments.
<<echo=TRUE>>=
f(c(3, 2), c(1, 1), 2)
f(y = c(1, 1), c(3, 2), 2)
f(y = c(1, 1), p = 2, c(3, 2))
...
résultat. Or, les règles d'interprétation d'un regroupement
d'expressions présentées à la
...
\cautionbox{Il existe une fonction \fonction{return} pour terminer
...
@
     \item Attention à la priorité des opérations! Elles sont
       effectuées dans cet ordre: inégalité, négation logique, «ou»
       logique.
<<echo=FALSE>>=
...
\begin{exercice}
  ...
  \begin{itemize}
  \item \fonction{sum} pour calculer la somme des éléments d'un
    vecteur;
  \item \fonction{prod} pour calculer le produit des éléments d'un
    vecteur;
  \item \fonction{max} pour calculer le maximum d'un vecteur;
  \item \fonction{abs} pour calculer la valeur absolue de chacun des
